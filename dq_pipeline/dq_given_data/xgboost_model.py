...
Results saved to output/xgb_results.json for comparator.

Why XGBoost over Random Forest?
    RF builds trees independently and averages.
    XGBoost builds trees sequentially — each tree corrects
    the errors of the previous one. More focused on hard cases.
"""
...
def train(X_train, y_train):
    """
    Imputes nulls and fits XGBoost classifier.
    No scaling needed — tree-based model like Random Forest.
    """
    imputer = SimpleImputer(strategy="median")
    X_train_imp = imputer.fit_transform(X_train)

    # Compute class imbalance ratio for scale_pos_weight
    # anomalies are 1.35% — ratio = normal_count / anomaly_count
    ...
    model = XGBClassifier(
        n_estimators=200,
        learning_rate=0.05,
        # each tree contributes 5% of its weight to the ensemble
        # slower learning = less overfitting
        max_depth=4,
        # shallower than RF (10) because boosting compounds depth
        # a depth-4 boosted tree is equivalent to a much deeper standalone tree
        scale_pos_weight=ratio,
        # XGBoost's equivalent of class_weight="balanced"
        # tells it anomaly rows are ratio-times more important
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 60)
    print("  XGBOOST — REAL DATA PIPELINE")
    print("=" * 60)
    ...
    print("\n  Results saved → output/xgb_results.json")
